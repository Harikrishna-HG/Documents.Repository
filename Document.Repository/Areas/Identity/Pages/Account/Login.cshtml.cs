...
namespace Document.Repository.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (user != null)
                    {
                        ...
                        {
                            // Check if the user has a college associated with their account
                            var collegeExists = _context.Colleges.Any(c => c.CreatedByUserId == user.Id);

                            if (collegeExists)
                            {
                                // Redirect to User/Index if a college already exists
                                return RedirectToAction("Index", "User");
                            }
                            else
                            {
                                // Redirect to Colleges/Create if no college exists
                                return RedirectToAction("Index", "User");
                            }
                        }
